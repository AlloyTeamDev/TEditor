...
;Jx().$package('TE', function(J){
     ...
     * @type {TLine}
     */
    var TLine = new J.Class({
        init: function(node){
            this.node = node;
        }
    });

    TLine.create = function(cssText){
        var node = document.createElement(LINE_NODE_TYPE);
        node.setAttribute('tline', 'tline');
        node.style.cssText = cssText || '';
        return new TLine(node);
    };

    TLine.isLine = function(node){
        return node.getAttribute('tline') === 'tline';
    };

    TLine.getLine = function(node){
        while(node && node !== this.body){
            if(TLine.isLine(node)){
                return node;
            }else{
     ...
     * @type {TWord}
     */
    var TWord = new J.Class({
        init: function(node){
            this.node = node;
        }
    })

    TWord.create = function(cssText){
        var node = document.createElement(WORD_NODE_TYPE);
        node.setAttribute('tword', 'tword');
        node.style.cssText = cssText || '';
        return new TWord(node);
    };

    TWord.isWord = function(node){
        return node.getAttribute('tword') === 'tword';
    };

    TWord.getWord = function(node){
        while(node && node !== this.body){
            if(TWord.isWord(node)){
                return node;
            }else{
                node = node.parentNode;
            }
     ...
     * @type {TRange}
     */
    var TRange = new J.Class({
        init: function(win, body){
            this.win = win;
            this.body = body;
            this.range = TE.util.getRange(win, body);
            if(!this.range){
                throw new Error('TRange: range is null!');
            }
            this.revise();
        },
        release: function(){
